...
const validatePassword = (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (password.length < 6 || password.length > 30) {
        res.status(400).json({ succes: false, message: 'Password should be 6-30 characters in length.' });
    }
    // Check if password has uppercase letter(s)
    if (!hasUppercase) {
        res.status(400).json({ succes: false, message: 'Password should contain at least one uppercase letter' });
    }

    // Check if password has lowercase letter(s)
    if (!hasLowercase) {
        res.status(400).json({ succes: false, message: 'Password should contain at least one lowercase letter' });
    }

    // Check if password has number(s)
    if (!hasNumber) {
        res.status(400).json({ succes: false, message: 'Password should contain at least one number' });
    }

    // Check if password has special character(s)
    if (!hasSpecialCharacter) {
        res.status(400).json({ succes: false, message: 'Password should contain at least one special character' });
    }
